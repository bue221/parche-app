...
Parche es un flyer de concierto: **negro sobre blanco** (claro) e invertido (oscuro). El color vive en artwork de eventos y en la pantalla de éxito (`confirm` `#7ffeb1`), que no cambia con el esquema.
...
Inter sustituye Favorit (UI). Antonio sustituye Foggy (**solo hero**, ~80px+). Tracking UI: `0.06em`. Pesos: 300, 400, 700 — no 500/600.
...
| Cards / imágenes | 8px | `rounded-cards` |
| Botones | 40px | `rounded-buttons` |

Pills: all-caps 12/700. Sin sombras ni degradados en chrome. Sin color en botones, bordes o texto (salvo `confirm` en éxito).
